...
void mars::DX11PositionColorTestPipeline::render(AVertexBuffer& vertexBuffer)
{
    ...
    UINT offset = 0;
    deviceContext->IASetVertexBuffers(0, 1, &dx11VertexBuffer.getBuffer(), &stride, &offset);

    // Set the input layout (not shown here, but should be created based on the vertex structure)
